  ...
  <div class="col-xs-12">
    <div class="table-header">
      ...
      <div class="widget-toolbar no-border pull-right">
        <a href="<?php echo site_url('jadwal/tambah')?>" id="tambah" role="button" class="btn btn-sm btn-success">
          <i class="fa fa-plus-circle bigger-120"></i>
          Tambah
        </a>
        <a href="#" id="hapus" role="button" class="btn btn-sm btn-danger" onclick="javascript:hapusData('tb_jadwal')">
          <i class="fa fa-trash bigger-120"></i>
          Kosongkan
        </a>
        <a href="<?php echo site_url('jadwal');?>" id="tambah" role="button" class="btn btn-sm btn-info" data-toggle="modal">
  ...
  .dataTables_info{display: none !important;}
    ...
    <div style="width:100%;" id="dynamic-table_wrapper" class="dataTables_wrapper form-inline no-footer">
      ...
      <!-- <table id="dynamic-table" class="table table-striped table-bordered table-hover"> -->
        ...
        <tbody>
          <?php
          $no=1; foreach ($tb_jadwal->result() as $rs):
          $status=$this->M_jadwal->getAllJadwal_by($rs->kd_prodi, $rs->semester, $rs->th_akademik);
            ?>
          <tr>
            <td><?php echo $no++; ?></td>
            <td><?php echo $rs->th_akademik; ?></td>
            <!-- <td><?php echo $rs->semester; ?></td> -->
            <td><?php echo $rs->kd_prodi.'-'.$rs->prodi; ?></td>
            <td><?php echo $status; ?> Jadwal</td>
          </tr>
        <?php endforeach; ?>
        </tbody>
      </table>
